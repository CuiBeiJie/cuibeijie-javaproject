...
import com.cbjprivilege.dao.SysAclModuleMapper;
import com.cbjprivilege.dao.SysDeptMapper;
import com.cbjprivilege.dto.AclDto;
import com.cbjprivilege.dto.AclModuleLevelDto;
import com.cbjprivilege.dto.DeptLevelDto;
import com.cbjprivilege.model.SysAcl;
import com.cbjprivilege.model.SysAclModule;
import com.cbjprivilege.model.SysDept;
import com.cbjprivilege.util.LevelUtil;
...
public class SysTreeService {
	...
	public List<AclModuleLevelDto> aclListToTree(List<AclDto> aclDtoList) {
		...
		for (AclDto sysAcl : aclDtoList) {
			if (sysAcl.getStatus() == 1) {
				moduleIdAclMap.put(sysAcl.getAclModuleId(), sysAcl);
			}
		}
	...
	public void bindAclsWithOrder(List<AclModuleLevelDto> aclModuleLevelList,
		...
		for (AclModuleLevelDto dto : aclModuleLevelList) {
			List<AclDto> aclDtoList = (List<AclDto>) moduleIdAclMap.get(dto
					.getId());
	...
	public List<AclModuleLevelDto> aclModuleListToTree(
			List<AclModuleLevelDto> dtoList) {
		if (CollectionUtils.isEmpty(dtoList)) {
			return Lists.newArrayList();
		}
		// level -> [aclmodule1, aclmodule2, ...] Map<String, List<Object>>
		Multimap<String, AclModuleLevelDto> levelAclModuleMap = ArrayListMultimap
				.create();
	...
	public void transformAclModuleTree(List<AclModuleLevelDto> dtoList,
			String level, Multimap<String, AclModuleLevelDto> levelAclModuleMap) {
		for (int i = 0; i < dtoList.size(); i++) {
			AclModuleLevelDto dto = dtoList.get(i);
			String nextLevel = LevelUtil.calculateLevel(level, dto.getId());
			List<AclModuleLevelDto> tempList = (List<AclModuleLevelDto>) levelAclModuleMap
					.get(nextLevel);
	...
	public List<DeptLevelDto> deptListToTree(List<DeptLevelDto> deptLevelList) {
		...
		Multimap<String, DeptLevelDto> levelDeptMap = ArrayListMultimap
				.create();
